...
import { getSession, updateSession, addToHistory, createSession, checkSessionState, setCurrentArea } from '../service';
import { loadInstalledApps, getAllApps, installGithubApp, uninstallGithubApp, getInstalledGithubAppUrls } from '../apps/appLoader';
  ...
  try {
    ...
    if (!session) {
      ...
      if (session.sessionId !== sessionId) {
        ...
        console.error(`Session created with different ID than provided: ${session.sessionId} vs ${sessionId}`);
        return NextResponse.json(
          { 
    ...
    console.log(`Processing command '${command}' for session ${sessionId} in area ${session.currentArea}`);
    ...
    console.log(`Command processed. Session area: ${session.currentArea}, Result area: ${result.area}`);
    
    // If area changed, update it
    if (result.area && result.area !== session.currentArea) {
      console.log(`Area changed from ${session.currentArea} to ${result.area} for session ${sessionId}`);
...
async function handleAppCommand(
  ...
  try {
    ...
    if (result.screen !== currentScreen) {
      const newArea = result.screen === null 
...
async function handleMainMenuCommands(cmd: string, session: BbsSession, apps?: Record<string, BbsApp>): Promise<CommandResult> {
  // Use passed apps or get the latest
  const currentApps = apps || getAllApps();
  ...
  console.log(`Handling main menu command: ${cmd}, App index: ${appIndex}, Available apps: ${appIds.length}`);
